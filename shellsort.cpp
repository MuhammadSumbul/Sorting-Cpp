...
void urutkan() {
    for (int gap = jml_mhs / 2; gap > 0; gap /= 2) {
        for (int a= gap;a < jml_mhs; a+= 1) {
            temp = nilai[a];
            int b;
            for (b =a;b >= gap and nilai[b - gap].nilai_akhir < temp.nilai_akhir;b-= gap) {
                nilai[b] = nilai[b- gap];
            }
            nilai[b] = temp;
        }
...
void tampilan(){
    ...
    for(int a = 0;a<jml_mhs;a++){
        ...
        gotoxy(5,4+a);cout<<nilai[a].nama_mhs<<endl;gotoxy(13,4+a);cout<<nilai[a].nim<<endl;
        gotoxy(26,4+a);cout<<nilai[a].Partisipasi<<endl;gotoxy(35,4+a);cout<<nilai[a].nilai_tugas<<endl;
        gotoxy(41,4+a);cout<<nilai[a].nilai_UTS<<endl;gotoxy(47,4+a);cout<<nilai[a].nilai_UAS<<endl;gotoxy(57,4+a);
        cout<<nilai[a].nilai_akhir<<endl;
    }
}
